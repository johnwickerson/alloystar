...
public class HOLTranslator implements ReturnVisitor<Expression, Proc, Decls, IntExpression> {
    ...
    HOLTranslator(Bounds bounds, Options options, AnnotatedNode<Formula> annotated) {
    	...
    	if (!annotated.hasHOLannotations()) annotated.annotateHOL();
	}

	@Override public Decls         visit(Decls decls)                  { return decls; }
    @Override public Decls         visit(Decl decl)                    { return decl; }
    @Override public Expression    visit(Relation relation)            { return relation; }
    @Override public Expression    visit(Variable variable)            { return variable; }
    @Override public Expression    visit(ConstantExpression constExpr) { return constExpr; }
    @Override public Expression    visit(UnaryExpression unaryExpr)    { return unaryExpr; }
    @Override public Expression    visit(BinaryExpression binExpr)     { return binExpr; }
    ...
    @Override public Expression    visit(ProjectExpression project)    { return project; }
    @Override public Expression    visit(IntToExprCast castExpr)       { return castExpr; }
    @Override public IntExpression visit(IntConstant intConst)         { return intConst; }
    @Override public IntExpression visit(IfIntExpression intExpr)      { return intExpr; }
    @Override public IntExpression visit(ExprToIntCast intExpr)        { return intExpr; }
    @Override public IntExpression visit(NaryIntExpression intExpr)    { return intExpr; }
    @Override public IntExpression visit(BinaryIntExpression intExpr)  { return intExpr; }
    @Override public IntExpression visit(UnaryIntExpression intExpr)   { return intExpr; }
    @Override public IntExpression visit(SumExpression intExpr)        { return intExpr; }
    @Override public Expression    visit(Comprehension comprehension)  { return comprehension; }

    @Override public Proc visit(IntComparisonFormula intComp)          { return new Proc.FOL(bounds, intComp); }
    @Override public Proc visit(ConstantFormula constant)              { return new Proc.FOL(bounds, constant); }
    @Override public Proc visit(ComparisonFormula cf)                  { return new Proc.FOL(bounds, cf); }
    @Override public Proc visit(MultiplicityFormula multFormula)       { return new Proc.FOL(bounds, multFormula); }
    @Override public Proc visit(RelationPredicate predicate)           { return new Proc.FOL(bounds, predicate); }
    @Override public Proc visit(NotFormula not)                        { assertNNF(not); return new Proc.FOL(bounds, not); }

    @Override
    public Proc visit(BinaryFormula binFormula) {
        if (annotated.isFOLNode(binFormula)) return new Proc.FOL(bounds, binFormula);
        Proc left, right;
    ...
    public static Proc toProc(Formula formula, Bounds bounds, Options options)                  { return toProc(annotate(formula), bounds, options); }
	...
	public static HOLTranslation proc2transl(Proc proc, Options options)            { return proc2transl(proc, options, 0); }
    ...
    /** Don't call this for translating the top-level formula; call Translator.translateHOL instead */
